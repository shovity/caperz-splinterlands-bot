...
function sortCards(a, b) {
    ...
    var a_index = colors.indexOf(a.color || a.card_details.color) * 1e4 + (a.rarity || a.card_details.rarity) * 1e3 + (a.id || a.card_details.id);
    var b_index = colors.indexOf(b.color || b.card_details.color) * 1e4 + (b.rarity || b.card_details.rarity) * 1e3 + (b.id || a.card_details.id);
...
function getEdition(edition) {
    return ["Alpha", "Beta", "Orb", "Reward", "Untamed", "Dice", "Gladius", "Chaos"][edition]
}
function getEditionByName(edition) {
    return ["ALPHA", "BETA", "ORB", "REWARD", "UNTAMED", "DICE", "GLADIUS", "CHAOS"].indexOf(edition.toUpperCase())
}
...
SM = {
    ...
    ShowPackOpenVideo: function(edition) {
        var video = $('<div id="divVideo" style="position:absolute; top: 0; z-index: 10000; width: 100vw;">' + '<video style="width: 100%;" onended="SM.PackOpenVideoEnded();" onclick="this.play()" autoplay>' + '<source src="https://d36mxiodymuqjm.cloudfront.net/website/vfx/pack-opening_' + (edition === 7 ? "7_hires" : edition) + '.mp4" type="video/mp4">' + "</video></div>");
        video.appendTo("body");
        if (!SM.Player.settings.mute_all) {
            let pack_open_audio = edition === 6 ? new Audio("https://d36mxiodymuqjm.cloudfront.net/website/sfx/SiteUI_GladiusCase_PackOpenAnimation.mp3") : edition === 7 ? new Audio("https://d36mxiodymuqjm.cloudfront.net/website/sfx/SiteUI_ChaosLegion_PackOpenAnimation.mp3") : new Audio("https://d36mxiodymuqjm.cloudfront.net/website/sfx/SiteUI_OpenPack_PackOpenAnimation.mp3");
        ...
        SM.Api(`/market/${url}`, {
            ...
            var data = response.map(r=>{
                ...
                return Object.assign({
                    ...
                    name: card.name,
                    type: card.type,
                    armor: card.type === "Monster" ? card.stats.armor[r.level - 1] : card.stats.armor,
                    attack: card.type === "Monster" ? card.stats.attack[r.level - 1] : card.stats.attack,
                    health: card.type === "Monster" ? card.stats.health[r.level - 1] : card.stats.health,
                    magic: card.type === "Monster" ? card.stats.magic[r.level - 1] : card.stats.magic,
                    ranged: card.type === "Monster" ? card.stats.ranged[r.level - 1] : card.stats.ranged,
                    speed: card.type === "Monster" ? card.stats.speed[r.level - 1] : card.stats.speed,
    ...
    OpenPack: function(edition) {
        ...
        SM.BroadcastCustomJson("sm_open_pack", "Open Pack", {
            ...
            if (!result || result.error) {
                ...
                var pack_balance = SM.Player.balances.find(b=>b.token === getEdition(edition).toUpperCase());
                if (pack_balance)
                    pack_balance.balance -= 1;
            ...
            if (result && result.trx_info && result.trx_info.result) {
                ...
                if (data && data.cards) {
                    ...
                    if (unrevealed && unrevealed.length > 0 && unrevealed.includes(result.trx_info.id)) {
    ...
    OpenAllPacks: function(edition, qty) {
        let pack_str = ["Alpha Packs", "Beta Packs", "Essence Orbs", "", "Untamed Packs", "AZMARE Dice", "Gladius Cases", "Chaos Legion Packs"];
    ...
    OnPurchaseComplete: function(purchase) {
        ...
        } else if (purchase.type == "booster_pack" || purchase.type == "cl_presale_pack" || purchase.type == "chaos_pack") {
            SM.HideDialog(true, function() {
                SM.ShowOpenPacks();
                window.scrollTo(0, 0);
                if (purchase.data) {
        ...
        gtag("event", "purchase", {
            transaction_id: purchase.purchase_id || purchase.id,
            affiliation: "Splinterlands Website - Shop",
            value: purchase.amount_usd,
            currency: "USD",
            items: [{
                id: purchase.type,
                name: purchase.type,
    ...
    MarketPurchase: function(market_ids, price, amount, currency, callback) {
        ...
        SM.ShowLoading();
        SM.MarketItemStillAvailable(market_ids, (still_available,not_available,err)=>{
            if (err) {
                alert(err);
                callback();
                return
            ...
            if (not_available.length > 0) {
                const not_available_card_list = not_available.map(m=>m.cards[0].uid).join(", ");
    ...
    MarketItemStillAvailable: function(market_ids, callback) {
        SM.Api("/market/status?ids=" + market_ids, null, function(data) {
            if (!data || !!data.error) {
                const err = "Error loading market status, please try your transaction again";
                if (callback)
                    callback([], [], err);
                return
            }
            ...
            if (callback)
                callback(still_available, not_available, null)
        })
    },
    LoadSettings: function(callback) {
        SM.Api("/settings", null, function(response) {
            if (!response || !response.version) {
                setTimeout(function() {
                    SM.Api("/settings", null, handleResponse)
                }, 2 * 1e3)
            } else {
    ...
    _transactions: {},
    TrxLookup: function(trx_id, is_api_tx, details, callback, retries, suppressError, timeout) {
        ...
        if (timeout > 0) {
            SM._transactions[trx_id].timeout = setTimeout(()=>{
                if (SM._transactions[trx_id] && SM._transactions[trx_id].status != "complete") {
                    if (!is_api_tx) {
                        alert("It appears that there has been a delay in processing this transaction from the blockchain. Please be patient and do not submit the transaction again or it will be processed twice. For pack openings, you will receive a notification in-game when your packs are ready.");
                        SM.HideLoading()
                    }
                    delete SM._transactions[trx_id];
    ...
    GetImageUrl(card, is_battle) {
        ...
        if (!details)
            return null;
        let urls = ["cards_v2.2", "cards_beta", "cards_v2.2", "cards_beta", "cards_untamed", "cards_untamed", "cards_gladiator", "cards_chaos"];
        let folder_url = urls[card.edition];
        let editions = ["alpha", "beta", "alpha", "beta", "untamed", "untamed", "gladiator", "chaos"];
        if (is_battle)
            folder_url = `cards_battle_${editions[card.edition]}`;
        return `${Config.card_image_url}/${folder_url}/` + (card.skin ? card.skin + "/" : "") + encodeURIComponent(details.name).replace(/'/g, "%27") + (card.gold ? "_gold" : "") + (details.tier >= 7 ? ".jpg" : ".png")
    },
    ...
    BroadcastCustomJson: async function(id, title, data, callback, retries, supressErrors) {
        ...
        if (SM.Player.use_proxy) {
            if (is_api_tx) {
                jQuery.post(`${Config.battle_api_url || Config.api_url}/battle/battle_tx`, {
                    signed_tx: JSON.stringify(tx)
                }, response=>{
                    if (response && response.id)
                        SM.TrxLookup(response.id, true, null, callback, 10, supressErrors);
                    else
        ...
        if (!active_auth || active_auth && window.hive_keychain) {
            try {
                ...
                if (response && response.id)
                    return SM.TrxLookup(response.id, is_api_tx, null, callback, 10, supressErrors);
    ...
    BroadcastCustomJsonLocal: function(id, title, data, callback, retries, supressErrors) {
        ...
        if (SM._use_keychain || active_auth && window.hive_keychain) {
            ...
            hive_keychain.requestCustomJson(SM.Player.name, id, active_auth ? "Active" : "Posting", JSON.stringify(data), title, function(response) {
                if (response.success) {
                    SM.TrxLookup(response.result.id, false, null, callback, 10, supressErrors)
                } else {
                    if (response.error == "user_cancel")
                        alert("Transaction was cancelled.");
                    else if (response.error && JSON.stringify(response.error).indexOf("Please wait to transact") >= 0) {
        ...
        callbacks: {
            onTemplate: function() {
                if (this.options.type === "challenge") {
                    ...
                    this.barDom.innerHTML = SM.ShowComponent("/notifications/pack_reveal", this.options.data)
                }
            }
        }
    })
    ...
    transaction_complete(data) {
        ...
        if (data.trx_info.type === "market_purchase") {
            ...
            gtag("event", "purchase", {
                ...
                value: parseFloat((total_usd * (SM.settings.market_fee / 1e4)).toFixed(2)),
                currency: "USD",
                items: [{
                    id: "market_purchase_fee",
                    name: "Market Purchase Fee",
                    price: parseFloat((total_usd * (SM.settings.market_fee / 1e4)).toFixed(2)),
        ...
        if (["open_pack", "open_all"].includes(data.trx_info.type) && timed_out && !data.error) {
...
SM.Guilds = function() {
    ...
    function ShowGuild(id, tab, extra_info) {
        SM.HighlightMenuItem("guilds");
        if (!extra_info || extra_info === "show-intro" && SM.Player && SM.Player.settings.skip_brawl_anim) {
            extra_info = "none"
        }
        SM.Api("/guilds/find", {
            id: id,
...
class SpineCharacter {
    constructor(targetContainer, assetName, animName, scale, x, y) {
        ...
        this.character_loader.add(targetContainer, `https://d36mxiodymuqjm.cloudfront.net/website/${assetName}`);
        this.container = new PIXI.Container;
        this.container.filterArea = this.portrait.screen;
        this.portrait.stage.addChild(this.container);
        this.animation = null;
        this.character_loader.onComplete.add((loader,resources)=>{
            this.animation = new PIXI.spine.Spine(this.character_loader.resources[targetContainer].spineData);
