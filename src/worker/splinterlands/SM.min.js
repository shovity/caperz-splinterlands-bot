...
function getEdition(edition) {
    return ["Alpha", "Beta", "Orb", "Reward", "Untamed", "Dice"][edition]
}
function getEditionByName(edition) {
    return ["ALPHA", "BETA", "ORB", "REWARD", "UNTAMED", "DICE"].indexOf(edition.toUpperCase())
}
...
SM = {
    ...
    ShowPackOpenVideo: function(edition) {
        ...
        if (!SM.Player.settings.mute_all) {
            let pack_open_audio = new Audio("https://d36mxiodymuqjm.cloudfront.net/website/sfx/SiteUI_OpenPack_PackOpenAnimation.mp3");
    ...
    ShowOpenPacks: function(source) {
        SM.HighlightMenuItem("open");
        if (SM.Player) {
            SM.ShowLoading();
            SM.LoadBalances(()=>SM.ShowHomeView("mana_well"), ()=>SM.HideLoading())
        } else
    ...
    OpenAllPacks: function(edition, qty) {
        let pack_str = ["Alpha Packs", "Beta Packs", "Essence Orbs", "", "Untamed Packs", "AZMARE Dice"];
    ...
    SubmitPayment: function(payment_method, amount, currency, purchase_id, data) {
        ...
        if (payment_method == "vessel") {
            ...
            SM.ShowLoading();
            let p = new Promise(resolve=>SM.TransferDEC(SM.settings.account, amount, {
                type: "pack_purchase",
                purchase_id: purchase_id
            }, r=>{
                if (!r || r.error)
    ...
    TransferDEC: function(to, qty, data, callback) {
        var obj = {
            to: to,
            qty: qty,
            token: "DEC"
        ...
        SM.BroadcastCustomJson("sm_token_transfer", "Transfer DEC", obj, result=>{
            if (callback)
                callback(result)
        }
        )
    ...
    GetImageUrl(card, is_battle) {
        ...
        if (!details)
            return null;
        let urls = ["cards_v2.2", "cards_beta", "cards_v2.2", "cards_beta", "cards_untamed", "cards_untamed"];
        let folder_url = urls[card.edition];
        let editions = ["alpha", "beta", "alpha", "beta", "untamed", "untamed"];
        if (is_battle)
    ...
    GetMaxMonsterLevel(league, summoner, monster_rarity) {
        if (league == 0)
            return 1;
        let rating_level = SM.settings.leagues[league].level;
    ...
    SubmitEnterTournament: async function(id, fee, signed_pw) {
        ...
        if (fee && getCurrency(fee) != "DEC") {
            ...
            if (fee && !confirm(`This tournament has an entry fee of ${fee} which will be deducted from your balance. Do you want to continue?`))
                return;
            if (parseFloat(fee) > 1e3) {
                SM.TransferDEC(SM.settings.account, parseFloat(fee).toFixed(3), {
    ...
    TournamentPayment: async function(tournament_id, payment) {
        ...
        } else if (currency.currency == "DEC") {
            SM.TransferDEC(SM.settings.account, parseFloat(payment).toFixed(3), {
                type: "tournament_payment",
                tournament_id: tournament_id
            }, ()=>SM.LoadBalances())
        }
...
Messenger = {
    ...
    tournament_started(data) {
        if (data.sub_format === "brawl") {
            ...
            if (SM.Player.guild) {
                ...
                sendNotification("Brawl Started!", "https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png", "Your guild's next Brawl has started. Click here to play!", ()=>SM.Guilds.ShowGuild(SM.Player.guild.id, "brawl"))
            }
        } else {
    ...
    brawl_fray_update(data) {
        if ($(`.brawl-body[brawl="${data.guild_id}::${data.tournament_id}"]`).length > 0) {
            refreshFrayState(data.guild_id, data.tournament_id, false)
        }
    },
...
class BattleTracker {
    constructor(jsonBattleResults, player1, player2) {
        this.roundIndex = 0;
...
var portrait_loader = new PIXI.loaders.Loader;
class SMBPortrait {
    constructor(targetContainer, teamInfo, playerInfo) {
        let divContainer = $(targetContainer);
        this.portrait = new PIXI.Application(252,226);
        ...
        portrait_loader.add(targetContainer, "https://d36mxiodymuqjm.cloudfront.net/website/battle/portraits/" + (teamInfo.summoner.skin ? teamInfo.summoner.skin + "/" : "") + summoner.name + ".png");
        ...
        portrait_loader.onComplete.add((loader,resources)=>{
            ...
            this.filter = new PIXI.Filter(null,portrait_loader.resources[targetContainer + "_background"].data);
            switch (teamInfo.color.toLowerCase()) {
            case "black":
                this.filter.uniforms.finalColorMulti = [1, 0, 1];
                break;
            case "blue":
                this.filter.uniforms.finalColorMulti = [0, 0, 1];
                break;
            case "red":
                this.filter.uniforms.finalColorMulti = [1, 0, 0];
                break;
            case "green":
                this.filter.uniforms.finalColorMulti = [0, 1, 0];
                break;
            case "gold":
                this.filter.uniforms.finalColorMulti = [1, 1, 0];
                break;
            case "white":
                this.filter.uniforms.finalColorMulti = [1, 1, 1];
                break
            }
            this.container.filters = [this.filter];
            this.portrait.ticker.add(delta=>{
                var v2 = this.filter.uniforms.resolution;
        ...
        $(divContainer.children()[1]).css("box-shadow", "0 0 50px 5px " + color);
        $(divContainer.children()[1]).css("border", "2px solid " + color);
        $(divContainer.children()[1]).attr("uid", teamInfo.summoner.uid);
        $(divContainer.children()[1]).attr("id", teamInfo.summoner.uid)
    }
    CleanUp() {
        if (this.sprite)
            this.sprite.destroy();
        ...
        portrait_loader = new PIXI.loaders.Loader;
        this.portrait = null;
        this.container = null;
        this.sprite = null;
        this.filter = null
